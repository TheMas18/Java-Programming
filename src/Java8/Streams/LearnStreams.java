...
//		 Arrays.stream(arr2).filter(n->n%2==0);
		 ...
		 * 
		 * // we can convert directly also Stream<Integer> direct = Stream.of(1, 2, 3,
		 * 4);
		 * 
		 * //we can do it using iterator also. //generate values in range //first arg is
		 * seed . as we used 0 it means start from 0 and go till the limit. //if we dont
		 * use limit it will create inifinte stream
		 * 
		 * Stream<Integer> limit = Stream.iterate(0, n->n+1).limit(20);
		 * 
		 * // this is also another way to generate. // hello will be generated till the
		 * limit we mentioned Stream<String> limit2 =
		...
		List<Integer> mappedList = evenNumber.stream().map(x->x/2).collect(Collectors.toList());
		...
		Integer integer1 = list.stream().max((a,b)->b-a).get();
		Integer integer2 = list.stream().min((a,b)->a-b).get();
		long integer3 = list.stream().count();
